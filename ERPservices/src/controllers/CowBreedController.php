<?php
    ...
    class CowBreedController extends Controller {
        ...
        public function getMonthDataList($condition, $regions){
            ...
            $toTime = $condition['YearTo'] . '-' . str_pad($condition['MonthTo'], 2, "0", STR_PAD_LEFT) . '-28';// .CowBreedController::getLastDayOfMonth($ymTo);
            //exit;
            ...
            // get master goal
            $MasterGoalList = MasterGoalService::getList('Y', 'ปัจจัยการเลี้ยงโค');
            ...
            for($i = 0; $i < $diffMonth; $i++){
                ...
                foreach ($regions as $key => $value) {
                    ...
                    foreach ($MasterGoalList as $k => $v) {
                        ...
                        $cow_breed_type_id = $v['id'];
                        
                        $region_id = $value['RegionID'];
                        $monthName = CowBreedController::getMonthName($curMonth);
                        ...
                        $Current = CowBreedService::getMainList($curYear, $curMonth, $region_id, $cow_breed_type_id);
                        $data['CurrentAmount'] = floatval($Current['sum_amount']);
                        $data['CurrentBaht'] = floatval($Current['sum_baht']);

                        $Before = CowBreedService::getMainList($beforeYear, $curMonth, $region_id, $cow_breed_type_id); 
        ...
        public function getData($request, $response, $args){
            try{
                ...
                if(!empty($id)){
                    $_Data = CowBreedService::getDataByID($id);
                }else{
                    $_Data = CowBreedService::getData($cooperative_id, $months, $years);
                }
        ...
        public function updateData($request, $response, $args){
            ...
            try{
                ...
                $id = CowBreedService::updateData($_Data);

 				foreach ($_Detail as $key => $value) {
 					$value['cow_breed_id'] = $id;
                	CowBreedService::updateDetailData($value);
                }
     ...
     //           
        ...
        public function removeDetailData($request, $response, $args){
            try{
                ...
                $params = $request->getParsedBody();
                $id = $params['obj']['id'];
                $result = CowBreedService::removeDetailData($id);
        ...
        public function updateDataApprove($request, $response, $args){
            ...
            try{
                ...
                // print_r($data );
                // exit;
                $result = CowBreedService::updateDataApprove($id, $data);
