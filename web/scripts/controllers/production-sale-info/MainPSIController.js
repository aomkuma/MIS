angular.module('e-homework').controller('MainPSIController', function($scope, $cookies, $filter, $state, $uibModal, HTTPService, IndexOverlayFactory) {
    //console.log('Hello !');
    ...
    $scope.loadApproveList = function(){
      ...
      //  IndexOverlayFactory.overlayShow();
        HTTPService.clientRequest('production-sale-info/list/approve', null).then(function(result){
            if(result.data.STATUS == 'OK'){
      ...
      var params = {'region' : $scope.PersonRegion};
        HTTPService.clientRequest('factory/list', params).then(function(result){
            if(result.data.STATUS == 'OK'){
                ...
                $scope.loadList('production-sale-info/list/main');
            }
            IndexOverlayFactory.overlayHide();
        });
    }
    ...
    $scope.loadListDetail = function(action, condition){
        var params = {
            'condition' : condition
        };
        IndexOverlayFactory.overlayShow();
    ...
    $scope.goUpdate = function(id){
        window.location.href = '#/production-sale-info/update/' + id;
    }
    ...
    $scope.viewDetail = function(condition){
        $scope.ViewType = 'DETAIL';
        $scope.loadListDetail('production-sale-info/list/detail', condition);
    }
    $scope.exportReport = function(data,condition){
       // console.log(DetailList, $scope.data_description);
        ...
        IndexOverlayFactory.overlayShow();
        HTTPService.clientRequest('production-sale-info/report', params).then(function(result){
            if(result.data.STATUS == 'OK'){
    ...
    // $scope.loadList('production-sale-info/list', '');
    IndexOverlayFactory.overlayHide();
