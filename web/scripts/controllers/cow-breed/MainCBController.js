angular.module('e-homework').controller('MainCBController', function($scope, $cookies, $filter, $state, $uibModal, HTTPService, IndexOverlayFactory) {
    //console.log('Hello !');
    ...
    $scope.loadApproveList = function(){
      ...
      //  IndexOverlayFactory.overlayShow();
        HTTPService.clientRequest('cow-breed/list/approve', null).then(function(result){
            if(result.data.STATUS == 'OK'){
    ...
    $scope.goUpdate = function(id){
        window.location.href = '#/cow-breed/update/' + id;
    }
    ...
    $scope.exportReport = function(data,condition){
       // console.log(DetailList, $scope.data_description);
        ...
        IndexOverlayFactory.overlayShow();
        HTTPService.clientRequest('cow-breed/report', params).then(function(result){
            if(result.data.STATUS == 'OK'){
