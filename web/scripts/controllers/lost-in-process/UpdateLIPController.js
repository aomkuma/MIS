angular.module('e-homework').controller('UpdateLIPController', function($scope, $cookies, $filter, $state, $routeParams, $uibModal, HTTPService, IndexOverlayFactory) {
    //console.log('Hello !');
    ...
    $scope.page_type = 'lost-in-process';
        ...
        HTTPService.clientRequest('factory/list', params).then(function(result){
            if(result.data.STATUS == 'OK'){
                ...
                if($scope.ID !== undefined && $scope.ID !== null){
                    $scope.loadData('lost-in-process/get', $scope.ID);
                }
            }
            IndexOverlayFactory.overlayHide();
        });
    ...
    $scope.loadDairyFarming = function(type, parent_id){
        ...
        IndexOverlayFactory.overlayShow();
        HTTPService.clientRequest('dairy-farming/list/lost-in-process', params).then(function(result){
            if(result.data.STATUS == 'OK'){
                if(type == 'MAIN'){
    ...
    $scope.loadMasterGoalList = function(){
        var params = {'actives':'Y', 'menu_type' : 'การสูญเสียในกระบวนการ', 'factory_id' : $scope.Data.factory_id};
        IndexOverlayFactory.overlayShow();
    ...
    $scope.loadData = function(action, id){
        ...
        HTTPService.clientRequest(action, params).then(function(result){
            if(result.data.STATUS == 'OK' && result.data.DATA.Data != null){
                ...
                $scope.DataDetailList = $scope.Data.lost_in_process_detail;
    ...
    $scope.save = function(Data, DataDetailList){
        ...
        IndexOverlayFactory.overlayShow();
        HTTPService.clientRequest('lost-in-process/update', params).then(function(result){
            if(result.data.STATUS == 'OK'){
                //alert('save success');
                // if($scope.ID !== undefined && $scope.ID !== null){
                    alert('บันทึกสำเร็จ');
                    window.location.href = '#/lost-in-process/update/' + result.data.DATA.id;
                // }else{
                //     location.reload();    
                // }
                IndexOverlayFactory.overlayHide();
    ...
    $scope.cancelUpdate = function(){
        window.location.href = '#/lost-in-process';
    }
    ...
    $scope.goSearch = function(){
        ...
        // $scope.setData();
        $scope.loadData('lost-in-process/get');
        
        // $scope.DataDetailList = [
        //     {
        //         'id':''
        //         ,'lost-in-process_id':'1'
        //         ,'amount':'2'
        //         ,'price':'200'
        //         ,'values':'400'
        //     },
    ...
    $scope.removeDetail = function(id, index){
        ...
        $scope.alertMessage = 'ข้อมูลจะถูกลบจากระบบทันที<br>ต้องการลบรายการนี้ ใช่หรือไม่ ?';
            ...
            modalInstance.result.then(function (valResult) {
                IndexOverlayFactory.overlayShow();
                var params = {'id' : id};
                HTTPService.clientRequest('lost-in-process/delete/detail', params).then(function(result){
                    if(result.data.STATUS == 'OK'){
                        $scope.DataDetailList.splice(index, 1);
                    }
                    IndexOverlayFactory.overlayHide();
    ...
    $scope.approve = function(Data, OrgType){
        ...
        modalInstance.result.then(function (valResult) {
            ...
            HTTPService.uploadRequest('lost-in-process/update/approve', params).then(function(result){
                console.log(result);
                if(result.data.STATUS == 'OK'){
                    alert('บันทึกสำเร็จ');
    ...
    $scope.getUploadLogList = function(){
        var params = {'id' : $scope.Data.id, 'menu_type' : 'lost-in-process'};
    ...
    $scope.uploadFile = function(Data, AttachFile ){
        ...
        IndexOverlayFactory.overlayShow();
        var params = {'Data' : Data, 'AttachFile' : AttachFile, 'menu_type' : 'lost-in-process', 'FileDate' : $scope.FileDate};
            HTTPService.uploadRequest('lost-in-process/upload', params).then(function(result){
                console.log(result);
                if(result.data.STATUS == 'OK'){
                    alert('อัพโหลดสำเร็จ');
                    window.location.href = '#/lost-in-process';///update/' + Data.id;
    ...
    $scope.exportTemplate = function(){
       // console.log(DetailList, $scope.data_description);
        ...
        IndexOverlayFactory.overlayShow();
        HTTPService.clientRequest('lost-in-process/load/template', params).then(function(result){
            if(result.data.STATUS == 'OK'){
