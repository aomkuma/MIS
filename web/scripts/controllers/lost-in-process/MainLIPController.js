angular.module('e-homework').controller('MainLIPController', function($scope, $cookies, $filter, $state, $uibModal, HTTPService, IndexOverlayFactory) {
    //console.log('Hello !');
    ...
    $scope.loadApproveList = function(){
      ...
      //  IndexOverlayFactory.overlayShow();
        HTTPService.clientRequest('lost-in-process/list/approve', null).then(function(result){
            if(result.data.STATUS == 'OK'){
      ...
      var params = {'region' : $scope.PersonRegion};
        HTTPService.clientRequest('factory/list', params).then(function(result){
            if(result.data.STATUS == 'OK'){
                ...
                $scope.loadList('lost-in-process/list/main');
            }
            IndexOverlayFactory.overlayHide();
        });
    }
    ...
    $scope.exportReport = function(data,condition){
       // console.log(DetailList, $scope.data_description);
        ...
        IndexOverlayFactory.overlayShow();
        HTTPService.clientRequest('lost-in-process/report', params).then(function(result){
            if(result.data.STATUS == 'OK'){
