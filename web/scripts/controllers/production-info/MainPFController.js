angular.module('e-homework').controller('MainPFController', function($scope, $cookies, $filter, $state, $uibModal, HTTPService, IndexOverlayFactory) {
    //console.log('Hello !');
    ...
    $scope.loadApproveList = function(){
      ...
      //  IndexOverlayFactory.overlayShow();
        HTTPService.clientRequest('production-info/list/approve', null).then(function(result){
            if(result.data.STATUS == 'OK'){
      ...
      var params = {'region' : $scope.PersonRegion};
        HTTPService.clientRequest('factory/list', params).then(function(result){
            if(result.data.STATUS == 'OK'){
                ...
                $scope.loadList('production-info/list/main');
            }
            IndexOverlayFactory.overlayHide();
        });
    }
    ...
    $scope.goUpdate = function(id){
        window.location.href = '#/production-info/update/' + id;
    }
    ...
    $scope.viewDetail = function(condition){
        ...
        $scope.loadListDetail('production-info/list/detail', condition);
    }
    $scope.exportReport = function(data,condition){
       // console.log(DetailList, $scope.data_description);
        ...
        IndexOverlayFactory.overlayShow();
        HTTPService.clientRequest('production-info/report', params).then(function(result){
            if(result.data.STATUS == 'OK'){
    ...
    // $scope.loadList('production-info/list', '');
    IndexOverlayFactory.overlayHide();
