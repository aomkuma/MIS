angular.module('e-homework').controller('UpdatePFController', function($scope, $cookies, $filter, $state, $routeParams, $uibModal, HTTPService, IndexOverlayFactory) {
        ...
        HTTPService.clientRequest('factory/list', null).then(function(result){
            if(result.data.STATUS == 'OK'){
                ...
                if($scope.ID !== undefined && $scope.ID !== null){
                    $scope.loadData('production-info/get', $scope.ID);
                }
            }
            IndexOverlayFactory.overlayHide();
        });
    ...
    $scope.loadDairyFarming = function(type, parent_id){
        ...
        IndexOverlayFactory.overlayShow();
        HTTPService.clientRequest('dairy-farming/list/production-info', params).then(function(result){
            if(result.data.STATUS == 'OK'){
                if(type == 'MAIN'){
        ...
        HTTPService.clientRequest(action, params).then(function(result){
            if(result.data.STATUS == 'OK' && result.data.DATA.Data != null){
                ...
                for(var i =0; i < $scope.DataDetailList.length; i++){
                   $scope.loadSubProductMilk($scope.DataDetailList[i].production_info_type1);
                $scope.loadProductMilkDetail($scope.DataDetailList[i].production_info_type2);
                }
                IndexOverlayFactory.overlayHide();
            }else{
                if($scope.Data.id != ''){
    ...
    $scope.save = function(Data, DataDetailList){
        ...
        IndexOverlayFactory.overlayShow();
        HTTPService.clientRequest('production-info/update', params).then(function(result){
            if(result.data.STATUS == 'OK'){
                //alert('save success');
                // if($scope.ID !== undefined && $scope.ID !== null){
                    alert('บันทึกสำเร็จ');
                    window.location.href = '#/production-info/update/' + result.data.DATA.id;
                // }else{
                //     location.reload();    
                // }
                IndexOverlayFactory.overlayHide();
    ...
    $scope.cancelUpdate = function(){
        window.location.href = '#/production-info';
    }
    ...
    $scope.goSearch = function(){
        ...
        // $scope.setData();
        $scope.loadData('production-info/get');
        
        // $scope.DataDetailList = [
        //     {
        //         'id':''
        //         ,'production-info_id':'1'
        //         ,'amount':'2'
        //         ,'price':'200'
        //         ,'values':'400'
        //     },
    ...
    $scope.removeDetail = function(id, index){
        ...
        $scope.alertMessage = 'ข้อมูลจะถูกลบจากระบบทันที<br>ต้องการลบรายการนี้ ใช่หรือไม่ ?';
            ...
            modalInstance.result.then(function (valResult) {
                IndexOverlayFactory.overlayShow();
                var params = {'id' : id};
                HTTPService.clientRequest('production-info/delete/detail', params).then(function(result){
                    if(result.data.STATUS == 'OK'){
                        $scope.DataDetailList.splice(index, 1);
                    }
                    IndexOverlayFactory.overlayHide();
    ...
    $scope.approve = function(Data, OrgType){
        ...
        modalInstance.result.then(function (valResult) {
            ...
            HTTPService.uploadRequest('production-info/update/approve', params).then(function(result){
                console.log(result);
                if(result.data.STATUS == 'OK'){
                    alert('บันทึกสำเร็จ');
