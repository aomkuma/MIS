angular.module('e-homework').controller('MainController', function($scope, $cookies, $filter, $state, $uibModal, HTTPService, IndexOverlayFactory) {
    ...
    $scope.goUpdate = function(id){
        window.location.href = '#/production-factor/update/' + id;
    }
    ...
    $scope.removeData = function(id){
        ...
        modalInstance.result.then(function (valResult) {
            IndexOverlayFactory.overlayShow();
            var params = {'id' : id};
            HTTPService.clientRequest('production-factor/delete', params).then(function(result){
                if(result.data.STATUS == 'OK'){
                    $scope.loadList('production-factor/list', '');
                }
                IndexOverlayFactory.overlayHide();
            });
        });
