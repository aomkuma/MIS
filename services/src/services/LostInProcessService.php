    ...
    class LostInProcessService {
        ...
        public static function getMainList($years, $months, $factory_id, $master_type_id){
            return LostInProcess::select(DB::raw("SUM(mis_lost_in_process_detail.amount) AS sum_amount"
                                        ,DB::raw("SUM(mis_lost_in_process_detail.value) AS sum_baht")
                                    )
        ...
        public static function getDataByID($id){
            return LostInProcess::where('id', $id)
                    //->with('mouHistories')
                    ->with(array('lost_in_processDetail' => function($query){
                        $query->orderBy('update_date', 'DESC');
                        $query->with('dairyFarming');
                        $query->with('subDairyFarming');
                        $query->with('lost_in_processItem');
                    }))
                    ->first();      
        }
        ...
        public static function removeData($id){
            LostInProcessItem::where('lost_in_process_id', $id)->delete();
        ...
        public static function removeDetailData($id){
            LostInProcessItem::where('lost_in_process_detail_id', $id)->delete();
