...
class SpermSaleController extends Controller {
    ...
    public function loadDataApprove($request, $response, $args) {
        try {
            ...
            $Data = SpermSaleService::loadDataApprove($user_session['UserID']);
    ...
    public function getMonthDataList($condition, $regions) {
        ...
        $toTime = $condition['YearTo'] . '-' . str_pad($condition['MonthTo'], 2, "0", STR_PAD_LEFT) . '-28'; // .SpermSaleController::getLastDayOfMonth($ym);
        //exit;
    ...
    public function getMonthDataListreport($condition, $regions) {
...
//                    , 'region_id' => $region_id
    ...
    public function getAnnuallyDataListreport($condition, $regions) {
        ...
        foreach ($mastesgoallist as $key => $value) {
            ...
            for ($j = 0; $j < 12; $j++) {
                ...
                $Current = SpermSaleService::getMainListforreport($curYear-$yearList[$j], $curMonth, $value['id']);
                $SumCurrentAmount += floatval($Current['sum_amount']);
                $SumCurrentBaht += floatval($Current['sum_baht']);

                $Before = SpermSaleService::getMainListforreport($beforeYear-$yearList[$j], $curMonth, $value['id']);
                $SumBeforeAmount += floatval($Before['sum_amount']);
                $SumBeforeBaht += floatval($Before['sum_baht']);
            }
    ...
    public function updateDataApprove($request, $response, $args){
            ...
            try{
                ...
                // print_r($data );
                // exit;
                $result = SpermSaleService::updateDataApprove($id, $data);
